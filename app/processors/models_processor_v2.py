...
# Import devices module for device handling
from app.helpers.devices import (
    get_available_devices, 
    parse_device_setting, 
    get_onnx_device_type_and_id,
    get_tensorrt_plugin_path,
    TENSORRT_AVAILABLE
)
...
class ModelsProcessorV2(QtCore.QObject):
    processing_complete = QtCore.Signal()
    model_loaded = QtCore.Signal()  # Signal emitted with Onnx InferenceSession

    def __init__(self, main_window: 'MainWindow', device_setting='cuda:0'):
        super().__init__()
        self.main_window = main_window
        
        # Initialize device settings using the devices module
        self.trt_ep_options = {
        ...
        # Parse device setting to get providers, CUDA device ID, and PyTorch device string
        self.providers, self.cuda_device_id, self.torch_device_string = parse_device_setting(
            device_setting, 
            self.trt_ep_options
        )
        
        # Set device and provider name based on the first provider
        self.device = self.torch_device_string
        if isinstance(self.providers[0], tuple):
            self.provider_name = self.providers[0][0]
        else:
            self.provider_name = self.providers[0]
            
        # Get TensorRT plugin path if available
        self.tensorrt_plugin_path = get_tensorrt_plugin_path() if TENSORRT_AVAILABLE else None
    ...
    def set_device(self, device_setting: str) -> str:
        """
        Set the device for model execution based on a device setting string.
        
        Args:
            device_setting: Device setting string (e.g., 'cuda:0', 'trt:0', 'cpu')
            
        Returns:
            The provider name that was set
        """
        # Parse device setting to get providers, CUDA device ID, and PyTorch device string
        self.providers, self.cuda_device_id, self.torch_device_string = parse_device_setting(
            device_setting, 
        ...
        # Set device based on the torch device string
        self.device = self.torch_device_string
        
        # Set provider name based on the first provider
        if isinstance(self.providers[0], tuple):
            self.provider_name = self.providers[0][0]
        else:
            self.provider_name = self.providers[0]
        ...
        # Clear models to force reloading on the new device
        self.delete_models()
    ...
    def load_model_trt(self, model_name, custom_plugin_path=None, precision='fp16', debug=False):
        ...
        if not os.path.exists(self.models_trt_path[model_name]):
            # Use the TensorRT plugin path from devices module if not provided
            if custom_plugin_path is None:
                custom_plugin_path = self.tensorrt_plugin_path
    ...
    def delete_models_trt(self):
        if TENSORRT_AVAILABLE:
            for model_data in models_trt_list:
                ...
                if isinstance(self.models_trt.get(model_name), TensorRTPredictor):
    ...
    def switch_providers_priority(self, provider_name):
        ...
        if provider_name == "TensorRT" or provider_name == "TensorRT-Engine":
            device_setting = "trt:0"
        ...
        if device_setting:
            return self.set_device(device_setting)
        return self.provider_name
    ...
    # Методы для работы с face_swappers
    def get_arcface_model(self, face_swapper_model): 
        if face_swapper_model in arcface_mapping_model_dict:
            return arcface_mapping_model_dict[face_swapper_model]
        else:
